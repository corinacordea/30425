...
public class Game {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		read.printFileContents(Constants.PATH_TO_RULES_FILE);
		...
		for (int i = 0; i < Constants.NUMBER_OF_ROUNDS; i++) {
			...
			mapOne.showResources();
			
			Thread.sleep(1000);
			System.out.println("[Game] What should " + minerOne.getName() + " do next? Dig(1) or sleep(0)?");
			...
			if (mapOne.consumeResources(resource)) {
				if (minerSlept) {
